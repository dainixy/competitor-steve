#!/usr/bin/env node
// capture_take.mjs — deterministic recording rig for the fable/capture-rig branch.
// Records idle or speaking takes against the current dev server on port 3000.
//
// Usage:
//   node capture_take.mjs idle   <run_label>          # 20 s idle take (no interaction)
//   node capture_take.mjs speak  <run_label> "<text>" # speaking take (sends fixed sentence)
//
// Output per take (in .scratch/avatar-launch/ledger/baseline-2026-07-02/<run_label>/):
//   clip.mp4        — final video (ffmpeg mux from webm)
//   probe_ts.json   — per-frame probe timeseries { t, jaw, bl, br, sm, lip, headY, camZ, camFov }
//   meta.json       — run metadata
//
// Requires: chromium playwright, ffmpeg at ~/.local/bin/ffmpeg.
// Login: graverisd@gmail.com / NadiaLive2026!
// Turnstile: test-mode keys are already in .env.local (XXXX.DUMMY.TOKEN passes bypass).
...
// CANDIDATE_LEDGER env var overrides the default baseline path.
// Set it to capture into a candidate run dir instead of the baseline.
const LEDGER = process.env.CANDIDATE_LEDGER || '/Users/admin/Documents/claude/drt-steve/.scratch/avatar-launch/ledger/baseline-2026-07-02';
const ALEXIS_ROOT = '/Users/admin/Documents/claude/alexis';
...
if (!mode || !['idle', 'speak'].includes(mode)) {
  console.error('Usage: node capture_take.mjs <idle|speak> <label> [question]');
  process.exit(1);
}
    ...
    try {
      if (typeof window.__nadiaProbe === 'function') {
        ...
        window.__probeTS.push({
          ...
          br: p.eyeBlinkRight,
          sm: p.mouthSmileLeft,
          smR: p.mouthSmileRight,
          ckL: p.cheekSquintLeft,
          ckR: p.cheekSquintRight,
          lip: p.lipsync,
          bi: p.browInnerUp,
        ...
        if (window.__probeTS.length > 12000) window.__probeTS.shift(); // max ~200 s at 60 fps
      }
    } catch (e) {}
...
// Dismiss age gate if shown
try { await page.click('button:has-text("I am 18 or older")', { timeout: 5000 }); } catch {}

// Wait for probe + run_demo_1 to be registered (avatar fully loaded)
console.log('[capture] waiting for avatar load (up to 3 min)...');
...
console.log('[capture] avatar ready, probe registered');

// Extra settle time so idle animations initialize
...
if (mode === 'idle') {
  // Idle take: just record for 20 seconds, no interaction
  console.log('[capture] idle mode — recording 20s');
  await page.waitForTimeout(20000);

} else {
  // Speaking take: trigger run_demo_1 with the fixed question
  console.log(`[capture] speak mode — question: ${question.slice(0, 60)}`);

  // run_demo_1 on this branch calls window.prompt() to get the question text.
  // We intercept the dialog and supply our fixed text.
  ...
  // Wait for lipsync to start and finish (TTS audio plays)
...
fs.writeFileSync(path.join(OUT, 'raw.webm'), Buffer.from(b64 || '', 'base64'));
...
console.log(`[capture] saved probe_ts=${probeTS.samples?.length} samples, webm=${(b64 || '').length} b64chars`);

await browser.close();

// --- Transcode to mp4 ---
const webm = path.join(OUT, 'raw.webm');
const mp4 = path.join(OUT, 'clip.mp4');
try {
  execSync(`"${FFMPEG}" -y -fflags +genpts -i "${webm}" -c:v libx264 -preset fast -crf 23 -c:a aac "${mp4}"`, { stdio: 'inherit' });
  console.log(`[capture] mp4 saved: ${mp4}`);
} catch (e) {
  console.log('[capture] ffmpeg failed:', e.message);
}
